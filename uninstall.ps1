...
# Removes the store's TRUST_FOLDER entry install.ps1 added to
...
$InitPy = Join-Path $EngineDir "memory_init.py"
if ($DryRun) {
    & $PyCmd $InitPy --revoke-agy --dry-run --store "$StoreDir"
} else {
    & $PyCmd $InitPy --revoke-agy --store "$StoreDir"
}
